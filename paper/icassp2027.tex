...
\title{DEG-Sparse: Gene-Specific Sparse Attention for\\ 
       Combinatorial Perturbation Response Prediction}
...
\begin{abstract}
Predicting transcriptional responses to genetic perturbations is a fundamental challenge in computational biology with direct implications for drug discovery and cell engineering. Existing models typically encode perturbations as dense additive vectors broadcast uniformly across all genes, failing to capture the sparse, gene-specific nature of perturbation effects. We introduce DEG-Sparse, a framework that replaces dense perturbation broadcasting with a gene-specific sparse cross-attention mechanism. Each perturbation selectively modulates only its differentially expressed gene (DEG) subnetwork---typically the top 10-25\% of genes---through an efficient O(N) element-wise gating operation. We further propose an Interaction Gradient Penalty that detects and penalizes non-additive effects in combinatorial perturbations. On the Norman benchmark, DEG-Sparse reduces overall MSE by 23\% (0.0098 to 0.0076) while maintaining Pearson correlation, with consistent improvements across all combinatorial prediction subsets (seen0: -24\%, seen1: -27\%, seen2: -10\%). Our approach requires no external data or architectural redesign, making it a practical plug-in improvement for existing perturbation models.
\end{abstract}

\begin{IEEEkeywords}
Computational biology, perturbation prediction, sparse attention, gene expression, signal processing
\end{IEEEkeywords}

\section{Introduction}
Single-cell RNA sequencing (scRNA-seq) with CRISPR-based perturbations enables systematic measurement of transcriptional responses to genetic interventions~\cite{norman2019}. Accurate computational prediction of these responses---known as the ``virtual cell'' problem~\cite{bunne2024}---could accelerate drug discovery and functional genomics by replacing expensive wet-lab screens with in silico experiments.
...
\subsection{DEG-Sparse Cross-Attention}
The key limitation of Equation~(2) is its isotropic nature: every gene receives the same perturbation modulation. We propose replacing the additive broadcast with a gene-specific sparse gating mechanism:
...
\subsection{Interaction Gradient Penalty}
...
We ablate each component to isolate contributions. Without DEG-Sparse attention (reverting to additive broadcast), MSE increases to 0.0083, confirming the core module's effectiveness. The Interaction Gradient Penalty provides an additional 3\% improvement on combo subsets.
